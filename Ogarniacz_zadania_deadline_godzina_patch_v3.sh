...
PATCH_ID="OGARNIACZ_TASK_DEADLINE_TIME_2026_08_27_V3"
STAMP="$(date +%Y%m%d-%H%M%S)"
PARENT_DIR="$(dirname "$REPO")"
BACKUP_DIR="$PARENT_DIR/.ogarniacz-patch-backup/task-deadline-time-v3-$STAMP"
...
note "============================================================"
note " Ogarniacz — zadania: deadline + godzina na osi (V3)"
note " Patch: $PATCH_ID"
note " Repo:  $REPO"
...
const PATCH_ID = 'OGARNIACZ_TASK_DEADLINE_TIME_2026_08_27_V3'
...
// V3: poprawione escapowanie regexów czasu/daty (\d, nie \\d).
// ------------------------------------------------------------
...
//    V3 używa punktacji i wielu sygnałów, nie wymaga jednego układu AST.
// ------------------------------------------------------------
...
if (!adapterPatched) {
  throw new Error(
    'V3 nadal nie znalazła bezpiecznego punktu mapowania zadania na oś. Nie zapisano zmian.'
  )
}
...
if ! OGARNIACZ_BACKUP_DIR="$BACKUP_DIR" OGARNIACZ_MANIFEST="$MANIFEST" node "$PATCHER"; then
  restore_patch
  fail "Patch V3 przerwał pracę i przywrócił pliki zmieniane przez siebie."
fi
note
...
if [[ "$VALIDATION_FAILED" -ne 0 ]]; then
  ...
  tail -n 160 "$LOG" || true
  fail "Walidacja V3 nie przeszła. Repo przywrócone względem zmian tego patcha."
fi
