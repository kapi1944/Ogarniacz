  ...
  return <section className="panel-ustawien-aplikacji" aria-labelledby="ustawienia-aplikacji-tytul">
    ...
    <div className="siatka-sekcji-ustawien">
      ...
      <SekcjaUstawien tytul="Pulpit" resetuj={() => resetujSekcje('pulpit')}>
        ...
        <label className="pole pole--pelne"><span>Limit alertów</span><input type="number" min="1" max="10" value={szkic.pulpit.limitAlertow} onChange={(e) => aktualizujSzkic({ ...szkic, pulpit: { ...szkic.pulpit, limitAlertow: Number(e.target.value) } })} /></label><div className="pole pole--pelne"><span>Kafelki Pulpitu</span><div className="lista-typow-szybkich">{szkic.pulpit.kafelki.map((k, indeks) => <div key={k.id}><label><input type="checkbox" checked={k.widoczny} onChange={(e) => aktualizujSzkic({ ...szkic, pulpit: { ...szkic.pulpit, kafelki: szkic.pulpit.kafelki.map((x) => x.id === k.id ? { ...x, widoczny: e.target.checked } : x) } })} />{k.typ}</label><select value={k.zakresCzasu} onChange={(e) => aktualizujSzkic({ ...szkic, pulpit: { ...szkic.pulpit, kafelki: szkic.pulpit.kafelki.map((x) => x.id === k.id ? { ...x, zakresCzasu: e.target.value as typeof k.zakresCzasu } : x) } })}><option value="today">Dzisiaj</option><option value="3d">3 dni</option><option value="7d">7 dni</option><option value="30d">30 dni</option></select><input aria-label={`Limit ${k.typ}`} type="number" min="1" max="10" value={k.limit} onChange={(e) => aktualizujSzkic({ ...szkic, pulpit: { ...szkic.pulpit, kafelki: szkic.pulpit.kafelki.map((x) => x.id === k.id ? { ...x, limit: Number(e.target.value) } : x) } })} /><button type="button" className="przycisk przycisk--maly" aria-label={`Przesuń ${k.typ} w górę`} disabled={indeks === 0} onClick={() => { const kafelki = [...szkic.pulpit.kafelki]; [kafelki[indeks - 1], kafelki[indeks]] = [kafelki[indeks], kafelki[indeks - 1]]; aktualizujSzkic({ ...szkic, pulpit: { ...szkic.pulpit, kafelki: kafelki.map((x, i) => ({ ...x, kolejnosc: i })) } }) }}>↑</button><button type="button" className="przycisk przycisk--maly" aria-label={`Przesuń ${k.typ} w dół`} disabled={indeks === szkic.pulpit.kafelki.length - 1} onClick={() => { const kafelki = [...szkic.pulpit.kafelki]; [kafelki[indeks], kafelki[indeks + 1]] = [kafelki[indeks + 1], kafelki[indeks]]; aktualizujSzkic({ ...szkic, pulpit: { ...szkic.pulpit, kafelki: kafelki.map((x, i) => ({ ...x, kolejnosc: i })) } }) }}>↓</button></div>)}</div></div>
      </SekcjaUstawien>
