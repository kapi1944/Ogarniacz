...
describe.sequential('Rejestr Zadania', () => {
  ...
  it('zachowuje stare zadanie, edytuje zwykłe pole i własną właściwość', async () => {
    ...
    const definicja = await utworzWlasnePoleRejestru({ rejestrId: 'zadania', etykieta: 'Klient', typ: 'tekst' })
    ...
    await waitFor(async () => expect(await pobierzRepozytorium('zadania').pobierz(zadanie.id)).toMatchObject({ tytul: 'Zmienione zadanie', polaWlasne: { [definicja.id]: 'Ogarniacz' } }))
  })
