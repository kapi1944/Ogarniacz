...
export function WidokPulpitu() {
  ...
  return <div className="widok widok-pulpitu">
    ...
    <section className="pulpit-sekcja--teraz centrum-dowodzenia">
      <Karta klasa="centrum-dowodzenia__glowna"><div className="centrum-dowodzenia__czas"><span>{poraDnia}</span><time dateTime={format(teraz, 'HH:mm')}>{format(teraz, 'HH:mm')}</time></div><div className="tytul-karty"><Clock3 aria-hidden="true" /><span>{elementTeraz?.stan === 'trwa' ? 'Teraz' : 'Najbliższe'}</span></div>{!elementTeraz ? <PustyStan tytul="Spokojny dzień" opis="Brak trwających i zaplanowanych elementów na dziś." akcja={<div className="plan-dnia__akcje"><button type="button" className="przycisk przycisk--maly" onClick={otworzSzybkieDodawanie}>Dodaj element</button><Link className="przycisk przycisk--tekstowy" to="/echo">Zapytaj Echo</Link></div>} /> : <div className="centrum-dowodzenia__element">{elementTeraz.element.referencjaZrodla ? <Link to={adresReferencjiZrodla(elementTeraz.element.referencjaZrodla)}><strong>{elementTeraz.element.godzina && `${elementTeraz.element.godzina} · `}{elementTeraz.element.tytul}</strong></Link> : <strong>{elementTeraz.element.godzina && `${elementTeraz.element.godzina} · `}{elementTeraz.element.tytul}</strong>}<small>{elementTeraz.stan === 'trwa' ? 'Trwa teraz' : 'Najbliższy krok'} · {opisElementuKafelka(elementTeraz.element)}</small></div>}<p className="centrum-dowodzenia__opis">{opisDnia}</p></Karta>
