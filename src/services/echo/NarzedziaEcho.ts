...
import type { DziennikEcho, Przypomnienie, Recepta, RyzykoDzialania, Skierowanie, Wizyta, WpisTerapii } from '../../domain/typy'
...
export function utworzDomyslnyRejestrNarzedziEcho(): RejestrNarzedziEcho {
  ...
  rejestr.zarejestruj({ nazwa: 'create_prescription', opis: 'Dodaje receptę z pozycjami do rejestru zdrowia.', schematArgumentow: z.object({ kod: z.string().trim().min(1).max(100).optional(), dataWystawienia: dataIso, terminRealizacji: dataIso.optional(), pozycje: z.array(z.object({ nazwaLeku: z.string().trim().min(1).max(160), ilosc: z.number().int().positive(), dawkowanie: z.string().max(500).optional() })).min(1) }), ryzyko: 'niskie', wykonaj: async (dane) => { const recepta: Recepta = { ...utworzMetadane(), kod: dane.kod, dataWystawienia: dane.dataWystawienia, terminRealizacji: dane.terminRealizacji, status: 'do_realizacji', pozycje: dane.pozycje.map((pozycja, indeks) => ({ id: `${Date.now()}-${indeks}`, ...pozycja, iloscZrealizowana: 0 })) }; await pobierzRepozytorium('recepty').zapisz(recepta); return { id: recepta.id, tytul: recepta.kod ? `Recepta ${recepta.kod}` : 'Recepta', pozycje: recepta.pozycje, typ: 'recepta' } } })
  rejestr.zarejestruj({ nazwa: 'realize_prescription_item', opis: 'Oznacza liczbę zrealizowanych sztuk pozycji recepty.', schematArgumentow: z.object({ receptaId: z.string().min(1), pozycjaId: z.string().min(1), iloscZrealizowana: z.number().int().nonnegative() }), ryzyko: 'niskie', wykonaj: async ({ receptaId, pozycjaId, iloscZrealizowana }) => { const repo = pobierzRepozytorium('recepty'); const recepta = await repo.pobierz(receptaId); if (!recepta) return null; const pozycje = recepta.pozycje.map((x) => x.id === pozycjaId ? { ...x, iloscZrealizowana: Math.min(x.ilosc, iloscZrealizowana) } : x); const status = pozycje.every((x) => x.iloscZrealizowana >= x.ilosc) ? 'zrealizowana' : pozycje.some((x) => x.iloscZrealizowana > 0) ? 'czesciowo_zrealizowana' : 'do_realizacji'; await repo.zapisz({ ...recepta, pozycje, status }); return { id: recepta.id, tytul: recepta.kod ? `Recepta ${recepta.kod}` : 'Recepta', pozycje, status, typ: 'recepta' } } })
