...
import { podsumujCashFlow } from '../FinanseService'
...
export function utworzDomyslnyRejestrNarzedziEcho(): RejestrNarzedziEcho {
  ...
  rejestr.zarejestruj({ nazwa: 'list_shopping', opis: 'Pobiera listy zakupów oraz ich niekupione pozycje.', schematArgumentow: z.object({ listaId: z.string().min(1).optional() }), ryzyko: 'niskie', wykonaj: async ({ listaId }) => { const [listy, pozycje] = await Promise.all([pobierzRepozytorium('listyZakupow').lista(), pobierzRepozytorium('pozycjeZakupow').lista()]); return listy.filter((lista) => !listaId || lista.id === listaId).map((lista) => ({ id: lista.id, tytul: lista.nazwa, sklep: lista.sklep, pozycje: pozycje.filter((pozycja) => pozycja.listaId === lista.id && !pozycja.kupione).map((pozycja) => ({ id: pozycja.id, nazwa: pozycja.nazwa, ilosc: pozycja.ilosc })), typ: 'zakupy' })) } })
  rejestr.zarejestruj({ nazwa: 'add_shopping_item', opis: 'Dodaje pozycję do istniejącej listy zakupów.', schematArgumentow: z.object({ listaId: z.string().min(1), nazwa: z.string().trim().min(1).max(160), ilosc: z.string().trim().max(40).optional(), kategoria: z.string().trim().max(80).optional() }), ryzyko: 'niskie', wykonaj: async ({ listaId, nazwa, ilosc, kategoria }) => { const lista = await pobierzRepozytorium('listyZakupow').pobierz(listaId); if (!lista) return null; const pozycja = { ...utworzMetadane(), listaId, nazwa, ilosc: ilosc ?? '1', kategoria, kupione: false }; await pobierzRepozytorium('pozycjeZakupow').zapisz(pozycja); return { id: pozycja.id, tytul: pozycja.nazwa, listaId, typ: 'zakupy' } } })

  rejestr.zarejestruj({ nazwa: 'finance_period_summary', opis: 'Wylicza cash flow wyłącznie z zapisanych transakcji, rachunków, płatności stałych i rat.', schematArgumentow: z.object({ miesiac: z.string().regex(/^\d{4}-\d{2}$/) }), ryzyko: 'niskie', wykonaj: async ({ miesiac }) => podsumujCashFlow(miesiac, await pobierzRepozytorium('wydatki').lista(), await pobierzRepozytorium('rachunki').lista(), await pobierzRepozytorium('platnosciStale').lista(), await pobierzRepozytorium('raty').lista()) })
  rejestr.zarejestruj({ nazwa: 'create_transaction', opis: 'Zapisuje przychód, wydatek lub transfer w obecnym rejestrze finansowym.', schematArgumentow: z.object({ opis: z.string().trim().min(1).max(160), kwota: z.number().positive(), data: dataIso, kategoria: z.string().trim().min(1).max(80), rodzaj: z.enum(['wydatek', 'przychod', 'transfer']).optional() }), ryzyko: 'umiarkowane', wykonaj: async (dane) => { const transakcja = { ...utworzMetadane(), ...dane, rodzaj: dane.rodzaj ?? 'wydatek' }; await pobierzRepozytorium('wydatki').zapisz(transakcja); return { id: transakcja.id, tytul: transakcja.opis, kwota: transakcja.kwota, data: transakcja.data, typ: 'finanse' } } })

  rejestr.zarejestruj({ nazwa: 'vehicle_status', opis: 'Pobiera przebieg, terminy serwisowe i ostatnie tankowania zapisanych pojazdów.', schematArgumentow: z.object({ pojazdId: z.string().min(1).optional() }), ryzyko: 'niskie', wykonaj: async ({ pojazdId }) => (await pobierzRepozytorium('pojazdy').lista()).filter((pojazd) => !pojazdId || pojazd.id === pojazdId).map((pojazd) => ({ id: pojazd.id, tytul: pojazd.nazwa, przebieg: pojazd.przebieg, serwis: pojazd.planowanySerwisData ?? pojazd.wymianaOlejuDo, ostatnieTankowanie: pojazd.tankowania?.at(-1), typ: 'samochod' })) })
  rejestr.zarejestruj({ nazwa: 'add_refuel', opis: 'Dodaje tankowanie do historii wskazanego samochodu.', schematArgumentow: z.object({ pojazdId: z.string().min(1), data: dataIso, przebieg: z.number().nonnegative(), litry: z.number().positive(), koszt: z.number().positive(), pelnyBak: z.boolean().optional() }), ryzyko: 'umiarkowane', wykonaj: async ({ pojazdId, ...dane }) => { const repo = pobierzRepozytorium('pojazdy'); const pojazd = await repo.pobierz(pojazdId); if (!pojazd) return null; const tankowanie = { id: crypto.randomUUID(), data: dane.data, przebieg: dane.przebieg, litry: dane.litry, cena: dane.koszt, pelnyBak: dane.pelnyBak }; await repo.zapisz({ ...pojazd, przebieg: Math.max(pojazd.przebieg ?? 0, dane.przebieg), tankowania: [...(pojazd.tankowania ?? []), tankowanie] }); return { id: pojazd.id, tytul: pojazd.nazwa, tankowanie, typ: 'samochod' } } })

  rejestr.zarejestruj({ nazwa: 'list_medication_doses', opis: 'Pobiera dzisiejsze dawki i przewidywane wyczerpanie zapasu leków.', schematArgumentow: z.object({ data: dataIso }), ryzyko: 'niskie', wykonaj: async ({ data }) => { const [leki, wpisy] = await Promise.all([pobierzRepozytorium('leki').lista(), pobierzRepozytorium('dziennikLekow').lista()]); return generujDawkiDnia(leki, wpisy, data).map((dawka) => ({ id: dawka.idWystapienia, lekId: dawka.lek.id, tytul: dawka.lek.nazwa, godzina: dawka.planowanaGodzina, status: dawka.status, wyczerpanie: przewidywanaDataWyczerpania(dawka.lek, data), typ: 'lek' })) } })
  rejestr.zarejestruj({ nazwa: 'mark_medication_dose', opis: 'Oznacza pojedynczą dawkę jako zażytą, odroczoną albo pominiętą.', schematArgumentow: z.object({ lekId: z.string().min(1), data: dataIso, godzina, status: z.enum(['zazyte', 'odroczone', 'pominiete']) }), ryzyko: 'niskie', wykonaj: async ({ lekId, data, godzina: oGodzinie, status }) => { const [lek, wpisy] = await Promise.all([pobierzRepozytorium('leki').pobierz(lekId), pobierzRepozytorium('dziennikLekow').lista()]); if (!lek) return null; const dawka = generujDawkiDnia([lek], wpisy, data).find((element) => element.planowanaGodzina === oGodzinie); if (!dawka) return null; const wpis = zapiszStatusDawki(dawka, status); await pobierzRepozytorium('dziennikLekow').zapisz(wpis); return { id: wpis.id, tytul: lek.nazwa, status, typ: 'lek' } } })

  rejestr.zarejestruj({ nazwa: 'list_habits', opis: 'Pobiera nawyki i ich regularność w ostatnich 30 dniach.', schematArgumentow: z.object({ data: dataIso.optional() }), ryzyko: 'niskie', wykonaj: async ({ data }) => { const doDnia = data ?? new Date().toISOString().slice(0, 10); const [nawyki, wpisy] = await Promise.all([pobierzRepozytorium('nawyki').lista(), pobierzRepozytorium('dziennikNawykow').lista()]); return nawyki.filter((nawyk) => nawyk.aktywny).map((nawyk) => ({ id: nawyk.id, tytul: nawyk.nazwa, ...statystykaNawyku(nawyk, wpisy, doDnia, 30), typ: 'nawyk' })) } })

  rejestr.zarejestruj({ nazwa: 'search_knowledge', opis: 'Wyszukuje notatki, dokumenty, pomysły i kontakty w lokalnych danych Ogarniacza.', schematArgumentow: z.object({ fraza: z.string().trim().min(2).max(160) }), ryzyko: 'niskie', wykonaj: async ({ fraza }) => { const slowa = uproscDoWyszukiwania(fraza); const pasuje = (tekst: string) => slowa.every((slowo) => uproscDoWyszukiwania(tekst).some((wartosc) => pasujaOdmiany(wartosc, slowo))); const [notatki, dokumenty, pomysly, kontakty] = await Promise.all([pobierzRepozytorium('notatki').lista(), pobierzRepozytorium('dokumenty').lista(), pobierzRepozytorium('pomysly').lista(), pobierzRepozytorium('kontakty').lista()]); return [...notatki.filter((x) => pasuje(`${x.tytul} ${x.tresc}`)).map((x) => ({ id: x.id, tytul: x.tytul, typ: 'notatka' })), ...dokumenty.filter((x) => pasuje(x.nazwa)).map((x) => ({ id: x.id, tytul: x.nazwa, typ: 'dokument' })), ...pomysly.filter((x) => pasuje(`${x.tytul} ${x.opis}`)).map((x) => ({ id: x.id, tytul: x.tytul, typ: 'pomysl' })), ...kontakty.filter((x) => pasuje(x.nazwa)).map((x) => ({ id: x.id, tytul: x.nazwa, typ: 'kontakt' }))] } })
