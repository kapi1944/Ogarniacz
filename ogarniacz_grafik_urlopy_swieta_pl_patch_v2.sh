...
PATCH_ID="OGARNIACZ_URLOPY_SWIETA_PL_2026_08_NODE_V2"
REPO="/c/GitHub/Projects/Ogarniacz/Ogarniacz"

echo "============================================================"
echo " Ogarniacz — Grafik: urlopy + polskie święta"
echo " Patch: ${PATCH_ID}"
echo " Wersja: bez Pythona, oparta wyłącznie o Node.js"
echo " Repo:  ${REPO}"
echo "============================================================"
...
if ! command -v node >/dev/null 2>&1; then
  ...
  echo "Projekt Ogarniacz wymaga Node.js/npm, więc uruchom ten patch w tym samym Git Bash,"
  echo "w którym działają polecenia: node --version oraz npm --version."
  exit 1
fi
...
const PATCH_ID = process.env.PATCH_ID
...
function replaceOnce(text, oldText, newText, label) {
  ...
  if (first !== last) throw new Error(`${label}: kotwica nie jest jednoznaczna`)
  return text.slice(0, first) + newText + text.slice(first + oldText.length)
}

function insertAfterRegex(text, regex, addition, label) {
  const match = regex.exec(text)
  if (!match) throw new Error(`${label}: nie znaleziono kotwicy`)
  const end = match.index + match[0].length
  return text.slice(0, end) + addition + text.slice(end)
}

console.log('== 1/6 Model danych: Urlop ==')

{
  const rel = 'src/domain/typy.ts'
  let text = read(rel)

  if (!text.includes('export interface Urlop extends EncjaBazowa')) {
    const model = `
...
`
    text = insertAfterRegex(
      text,
      /export interface WyjatekGrafiku extends EncjaBazowa \{[\s\S]*?\n\}\n/,
      '\n' + model,
      'typy.ts / Urlop',
    )
  }

  if (!text.includes('  urlopy: Urlop\n')) {
    text = replaceOnce(
      text,
...
console.log('== 2/6 Migracja bazy Dexie ==')

{
  const rel = 'src/data/BazaOgarniacza.ts'
  let text = read(rel)

  if (!text.includes("  'urlopy',\n")) {
    text = replaceOnce(
      text,
      "  'wyjatkiGrafiku',\n",
      "  'wyjatkiGrafiku',\n  'urlopy',\n",
  ...
  const schema = "urlopy: 'id, dataOd, dataDo, typ, status, updatedAt, usunietoAt'"

  if (!text.includes(schema)) {
    const versions = [...text.matchAll(/this\.version\((\d+)\)/g)].map((m) => Number(m[1]))
    if (!versions.length) throw new Error('BazaOgarniacza.ts: nie znaleziono wersji Dexie')
    const nextVersion = Math.max(...versions) + 1

    const anchor = '\n  }\n\n  tabela<K extends NazwaTabeli>'
    if (!text.includes(anchor)) {
      throw new Error('BazaOgarniacza.ts: nie znaleziono końca konstruktora')
    }
    ...
    text = replaceOnce(
      ...
      anchor,
      migration + anchor,
      'BazaOgarniacza.ts / migracja urlopów',
    )

    console.log(`  + dodano wersję Dexie ${nextVersion}`)
  }

  write(rel, text)
}

console.log('== 3/6 Serwisy świąt i urlopów ==')

write('src/services/PolskieSwietaService.ts', `// ${PATCH_ID}
...
write('src/services/UrlopyService.ts', `// ${PATCH_ID}
...
write('src/services/KalendarzPracyService.test.ts', `import { describe, expect, it } from 'vitest'
...
console.log('== 4/6 Integracja z Planerem ==')

{
  const rel = 'src/services/PlanerService.ts'
  let text = read(rel)

  const oldImport = "import type { BlokCzasu, GrafikPracy, Nawyk, Wizyta, WyjatekGrafiku, Zadanie } from '../domain/typy'"
  if (text.includes(oldImport)) {
    text = text.replace(
      oldImport,
      "import type { BlokCzasu, GrafikPracy, Nawyk, Urlop, Wizyta, WyjatekGrafiku, Zadanie } from '../domain/typy'",
    )
  } else if (!text.includes('Urlop')) {
    throw new Error('PlanerService.ts: nie rozpoznano importu typów')
  }

  if (!text.includes("from './PolskieSwietaService'")) {
    text = replaceOnce(
      text,
  ...
  if (!text.includes('  urlopy: Urlop[]\n')) {
    text = replaceOnce(
      ...
      '  wyjatkiGrafiku: WyjatekGrafiku[]\n',
      '  wyjatkiGrafiku: WyjatekGrafiku[]\n  urlopy: Urlop[]\n',
      'PlanerService.ts / DanePlanera.urlopy',
    )
  }

  const holidayRule = 'dane.urlopy.some((urlop) => czyDataWUrlopie(urlop, dane.data))'
  if (!text.includes(holidayRule)) {
    const exceptionLine =
    ...
    text = replaceOnce(
      ...
      'PlanerService.ts / urlop i święto',
    )
  }
...
{
  const rel = 'src/services/PlanerService.test.ts'
  let text = read(rel)
  ...
  if (!text.includes('traktuje urlop jako dzień bez bloku pracy')) {
    const tests = `
...
`
    ...
    if (end < 0) throw new Error('PlanerService.test.ts: nie znaleziono końca describe')
    text = text.slice(0, end) + tests + text.slice(end)
  }

  write(rel, text)
}

console.log('== 5/6 Kalendarz i urlopy w module Grafik ==')

{
  const rel = 'src/modules/czas/WidokiCzasu.tsx'
  let text = read(rel)

  if (!text.includes('eachDayOfInterval')) {
    text = replaceOnce(
      text,
  ...
  const oldTypeImport = "import type { BlokCzasu, GrafikPracy, WyjatekGrafiku } from '../../domain/typy'\n"
  if (text.includes(oldTypeImport)) {
    text = text.replace(
      oldTypeImport,
      "import type { BlokCzasu, GrafikPracy, Urlop, WyjatekGrafiku } from '../../domain/typy'\n",
    )
  } else if (!text.includes('Urlop')) {
    throw new Error('WidokiCzasu.tsx: nie rozpoznano importu typów')
  }

  if (!text.includes("from '../../services/PolskieSwietaService'")) {
    text = replaceOnce(
      text,
  ...
  if (!text.includes("const { dane: urlopy } = useRepozytorium('urlopy')")) {
    text = replaceOnce(
      text,
  ...
  if (!text.includes('wyjatkiGrafiku: wyjatki, urlopy, odGodziny')) {
    text = replaceOnce(
      text,
      'wyjatkiGrafiku: wyjatki, odGodziny',
      'wyjatkiGrafiku: wyjatki, urlopy, odGodziny',
      'WidokiCzasu.tsx / przekazanie urlopów',
    )
  }

  if (!text.includes('function SekcjaKalendarzaGrafiku()')) {
...
`
    ...
    text = replaceOnce(
      ...
      component + 'export function WidokGrafiku() {',
      'WidokiCzasu.tsx / kalendarz',
    )
  }

  if (!text.includes('<SekcjaKalendarzaGrafiku />')) {
    const oldHeader =
...
console.log('== 6/6 Style kalendarza ==')

{
  const rel = 'src/styles/glowny.css'
  let text = read(rel)
  const marker = `/* ${PATCH_ID} */`
...
console.log('== Kontrola kompletności ==')

const checks = [
  ['model Urlop', read('src/domain/typy.ts').includes('export interface Urlop extends EncjaBazowa')],
  ['MapaTabel.urlopy', read('src/domain/typy.ts').includes('  urlopy: Urlop\n')],
  ['Dexie urlopy', read('src/data/BazaOgarniacza.ts').includes("urlopy: 'id, dataOd, dataDo, typ, status, updatedAt, usunietoAt'")],
  ['serwis świąt', fs.existsSync(file('src/services/PolskieSwietaService.ts'))],
  ['serwis urlopów', fs.existsSync(file('src/services/UrlopyService.ts'))],
  ['kalendarz', read('src/modules/czas/WidokiCzasu.tsx').includes('function SekcjaKalendarzaGrafiku()')],
  ['Planer', read('src/services/PlanerService.ts').includes('  urlopy: Urlop[]\n')],
  ['CSS', read('src/styles/glowny.css').includes(PATCH_ID)],
]

const missing = checks.filter(([, ok]) => !ok).map(([name]) => name)
if (missing.length) throw new Error(`Niekompletny patch: ${missing.join(', ')}`)
...
echo
echo "Dodano:"
echo "  - model i tabelę urlopów"
echo "  - urlop Od/Do + rodzaj + status + opis"
echo "  - miesięczny kalendarz Grafiku"
echo "  - polskie święta stałe i ruchome"
echo "  - Wigilię 24 grudnia jako ustawowy dzień wolny od 2025 r."
echo "  - oznaczenia święta/urlop/wyjątek na kalendarzu"
echo "  - integrację urlopów i świąt z Planerem"
echo "  - testy świąt, urlopów i Planera"
echo
echo "Backup:"
